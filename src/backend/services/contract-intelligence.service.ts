...
const CLAUSE_PATTERNS: Record<ClauseType, RegExp[]> = {
  ...
  auto_renewal: [
    ...
    /\b(annual(ly)?|annual basis)\b.*\b(renew|cancel)\b|\b(cancel)\b.*\b(renew|annual)\b/i,
  ],
  unilateral_modification: [
...
const RED_FLAG_RULES: RedFlagRule[] = [
  {
    ...
    pattern: /\b(binding arbitration|mandatory arbitration)\b[\s\S]*\b(waiv(e|er)|prohibit(ed)?|no class)\b/i,
  ...
  },
  {
    ...
    pattern: /\b(client|customer|borrower)\b.{0,200}\bindemnif(y|ies|ied)\b.{0,300}\b(negligence|gross negligence|willful misconduct)\b/i,
  ...
  },
  {
    ...
    pattern: /\b(governed by|jurisdiction).{0,100}(Nevada|Utah|Wyoming).{0,100}(arbitration|litigation|dispute)/i,
