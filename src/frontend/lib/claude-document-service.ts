...
const MOCK_TEMPLATES: Record<DocumentType, (ctx: Record<string, unknown>) => string> = {
  ...
  application_cover_letter: (ctx) => `Application Cover Letter\n\nDate: ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}\n\nTo: ${ctx.issuer ?? 'Issuer'} Business Card Division\nRe: Business Credit Card Application — ${ctx.card_name ?? 'Business Credit Card'}\nApplicant: ${ctx.client_name ?? '[Business Name]'}\n\nDear Underwriting Team,\n\nI am submitting this application for the ${ctx.card_name ?? '[card product]'} on behalf of ${ctx.client_name ?? '[Business Name]'}, a ${ctx.business_type ?? '[entity type]'} operating in the ${ctx.industry ?? '[industry]'} sector.\n\nBusiness Overview:\n- Annual Revenue: ${ctx.annual_revenue ? '$' + Number(ctx.annual_revenue).toLocaleString() : '[revenue]'}\n- Years in Business: ${ctx.years_in_business ?? '[years]'}\n- Number of Employees: ${ctx.employee_count ?? '[count]'}\n\nPurpose of Credit:\nThe requested credit line of ${ctx.requested_amount ? '$' + Number(ctx.requested_amount).toLocaleString() : '[amount]'} will be used for ${ctx.intended_use ?? 'operational expenses, vendor payments, and working capital needs'}.\n\nWe believe ${ctx.client_name ?? 'the business'} is well-positioned to manage this credit facility responsibly. Supporting documentation including bank statements, tax returns, and business financials are available upon request.\n\nThank you for your consideration.\n\nSincerely,\n${ctx.advisor_name ?? '[Advisor Name]'}\n${ctx.advisor_company ?? 'CapitalForge'}`,
