...
3. **Every state change is an event** — The canonical ledger provides a complete, tamper-evident chain of custody for any regulatory inquiry or litigation hold. **One known discontinuity:** brokered calls for the `burkham-wickmont` venture between 2026-09-03 and 2026-09-04 are recorded against the Demo Advisors tenant, not Burkham's. The rows are left as written — an append-only ledger is corrected by appending an explanation, not by editing it. Before reconstructing that venture's history, read [`docs/backlog/incident-2026-09-03-office-venture-mapped-to-demo-tenant.md`](backlog/incident-2026-09-03-office-venture-mapped-to-demo-tenant.md), which states the boundary: seed fixtures only, no real client data.
