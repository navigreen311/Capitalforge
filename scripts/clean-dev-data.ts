...
e2e workflow rules         : ${e2eWorkflows.length}`);
  ...
  // Compliance checks generated by a sweep.
  //
  // Every E2E run calls run-all or run-checks, each of which writes a check
  // row per business per type. Cleanup below removes checks belonging to
  // businesses the suite created, but the sweeps run against the seeded
  // clients, so those rows accumulated indefinitely — 262 of them by the time
  // this was noticed, which is past the take: 200 that /compliance/overview
  // reads with. A count-based assertion then stops being able to see a write
  // at all, which is exactly how it surfaced.
  //
  // Seeded checks carry seed-cc- ids and stay; anything else came from a
  // sweep and is safe to drop.
  const sweptChecks = await prisma.complianceCheck.findMany({
    where: { NOT: { id: { startsWith: 'seed-cc-' } } },
    select: { id: true },
  });
  if (sweptChecks.length > 0) {
    note('sweptComplianceChecks', (await prisma.complianceCheck.deleteMany({
      where: { id: { in: sweptChecks.map((c) => c.id) } },
    })).count);
  }
